...
class Interceptor {
  ...
  async init () {
    ...
    await Network.requestWillBeSent(async function ({requestId, request}) {
      debug('Request will be sent for: ', request.url)
      const responseReceivedWaiting = interceptor.requests[requestId]
      ...
      if (request.method !== 'POST') {
        ...
        interceptor.requests[requestId] = -1
        return
      }
